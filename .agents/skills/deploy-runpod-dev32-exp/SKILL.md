---
name: deploy-runpod-dev32-exp
description: Deploy the HAG4R MV2-Dev32 experiment on this Runpod machine. Use when an agent must run or resume the local Runpod Dev32 pipeline from `/workspace/HAG4R-runpod` with RealSim diagnostics, including mandatory pre-run updates of both HAG4R and `/workspace/RealSim-Hetero_Asset_Gen`, rebuilding `RealSimLiveServer`, validating local conda/data/credential paths, launching a smoke asset, proceeding to the full 32-asset batch even if smoke fails unless dependencies or credentials are missing, monitoring progress, and archiving or reporting results without using Slurm.
---

# Deploy Runpod Dev32 Exp

Use this skill for the Runpod deployment only. The core invariant is ordering: pull latest HAG4R, pull latest RealSim-Hetero, rebuild `RealSimLiveServer`, then run the experiment. Do not start a smoke or full Dev32 run until those three update/build steps have succeeded in the current session. Treat smoke as an early signal, not as a hard gate: proceed to the full Dev32 batch after smoke unless smoke proves missing dependencies or missing credentials.
...
- Dev32 input root: `/workspace/data/mvi2_00_mv2-dev32`
...
export DEV32_INPUT_ROOT=/workspace/data/mvi2_00_mv2-dev32
...
The referenced Runpod procedure used the paper-pinwheel asset:

```text
/workspace/data/mvi2_00_mv2-dev32/class_566_paper_pinwheel_instance_4e03bde8/006.jpg
```
...
Do these steps in order. If a tracked dirty worktree blocks `git pull --ff-only`, stop and report the exact dirty files instead of stashing, resetting, or overwriting user changes.
...
Fail loudly on missing environment, data, credentials, or GPU resources. Do not guess substitute paths for Dev32 input data or RealSim.

```bash
cd "$HAG4R_ROOT"
test -d "$HAG4R_ENV/conda-meta"
test -d "$REALSIM_ENV/conda-meta"
test -d "$DEV32_INPUT_ROOT"
test -f configs/config_gpt55_for_all.yaml
test -f outputs/build_mvimgnet_MV2_Dev32/MV2-Dev32.json
test -x "$REALSIM_ROOT/build/bin/RealSimLiveServer"
find "$DEV32_INPUT_ROOT" -mindepth 2 -maxdepth 2 -type f \( -iname '*.jpg' -o -iname '*.jpeg' -o -iname '*.png' \) | sort | wc -l
```
...
For `scripts/run_mvimgnet2_dev32_pipeline.py`, require visible GPUs in pairs. Use one two-GPU worker on a 2-GPU Runpod. On a 4- or 8-GPU Runpod, use one shard per two-GPU pair. If exactly one GPU is visible, proceed with the slower direct single-pipeline fallback from the prior Runpod procedure instead of stopping. If no GPU is visible, treat it as a missing dependency and stop before full Dev32.
...
SMOKE_IMAGE="$DEV32_INPUT_ROOT/class_566_paper_pinwheel_instance_4e03bde8/006.jpg"
SMOKE_RUN_ID="dev32_runpod_smoke_${STAMP}_paper_pinwheel"
mkdir -p .agents/logs
...
- Stop before full Dev32 only when the smoke failure proves missing dependencies or missing credentials, such as absent conda envs, unavailable required Python packages, missing model/checkpoint assets, missing `OPENAI_API_HAG4R_KEY`, missing RealSim binary/build output, or an unavailable required data/config path.
- Proceed to the full Dev32 batch for all other smoke failures. This includes asset-specific failures, model/tool mistakes, timeouts, segmentation or OmniPart failures, RealSim diagnostics failures, final artifact mismatches, and unexplained nonzero exits after dependencies and credentials were already preflighted.

Record the smoke failure signature, smoke log path, and why it is not a dependency/credential blocker in the final report.

## Full Dev32 Run

Prefer the checked-in Dev32 runner for the full experiment. Launch it after the smoke attempt unless dependencies or credentials are missing. It validates the manifest, records JSONL ledgers, runs every selected asset through `hag4r.agentic.run_pipeline`, captures per-asset logs, checks final export artifacts, and records GPU binding evidence.
...
- Config, Dev32 input root, HAG4R env, RealSim env, GPU bindings, and cache settings.
...
Create archives only for canonical Dev32 run outputs that belong to this deployment, excluding smoke-only folders unless the user asks to keep them together:

```bash
COMMIT="$(git -C "$HAG4R_ROOT" rev-parse --short HEAD)"
DATE="$(date -u +%F)"
zip -r "outputs/agentic_asset_refinement/runpod_dev32_${DATE}_${COMMIT}_full.zip" outputs/agentic_asset_refinement/<run-id-prefixes-or-list>
zip -r "outputs/run_pipeline/runpod_dev32_${DATE}_${COMMIT}_full.zip" outputs/run_pipeline/<output-tag-prefixes-or-list>
```
